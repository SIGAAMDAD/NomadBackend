...
namespace NomadCore.Systems.SaveSystem.Infrastructure.Streams {
	/*
	===================================================================================
		...
		*/
		...
		private static long GetFreeDiskSpace( string path ) {
			try {
				...
				ServiceRegistry.Get<ILoggerService>().PrintError( $"SaveStreamWriter.GetFreeDiskSpace: error getting remaining disk space in drive {path}" );
			}
			return -1;
		}
	};
