...
{
    public sealed class PackedBitSet8SparseCache
    {
        private const int PageShift = 3;
        private const int PageMask = 7;

        private readonly Dictionary<int, int> _slotsByPage;
        private int[] _pageKeys;
        private PackedBitSet8[] _pages;
        private bool[] _dirty;
        private int _count;
        ...
        public PackedBitSet8SparseCache(int initialPageCapacity = 16)
        {
            ...
            _pages = new PackedBitSet8[initialPageCapacity];
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public ref PackedBitSet8 GetPageBySlot(int slot)
        {
            Debug.Assert((uint)slot < (uint)_count);
