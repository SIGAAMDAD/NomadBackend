...
{
    public sealed class PackedBitSet256SparseCache
    {
        private const int PageShift = 8;
        private const int PageMask = 255;

        private readonly Dictionary<int, int> _slotsByPage;
        private int[] _pageKeys;
        private PackedBitSet256[] _pages;
        private bool[] _dirty;
        private int _count;
        ...
        public PackedBitSet256SparseCache(int initialPageCapacity = 16)
        {
            ...
            _pages = new PackedBitSet256[initialPageCapacity];
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public ref PackedBitSet256 GetPageBySlot(int slot)
        {
            Debug.Assert((uint)slot < (uint)_count);
