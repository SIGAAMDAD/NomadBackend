...
{
    public sealed class PackedBitSet128SparseCache
    {
        private const int PageShift = 7;
        private const int PageMask = 127;

        private readonly Dictionary<int, int> _slotsByPage;
        private int[] _pageKeys;
        private PackedBitSet128[] _pages;
        private bool[] _dirty;
        private int _count;
        ...
        public PackedBitSet128SparseCache(int initialPageCapacity = 16)
        {
            ...
            _pages = new PackedBitSet128[initialPageCapacity];
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public ref PackedBitSet128 GetPageBySlot(int slot)
        {
            Debug.Assert((uint)slot < (uint)_count);
