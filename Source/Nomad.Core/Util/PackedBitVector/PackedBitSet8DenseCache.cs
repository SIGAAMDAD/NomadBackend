...
{
    public sealed class PackedBitSet8DenseCache
    {
        private const int PageShift = 3;
        private const int PageMask = 7;

        private readonly PackedBitSet8[] _pages;
        ...
        public PackedBitSet8DenseCache(int maxBitId)
        {
            if (maxBitId < 0)
            {
            ...
            _pages = new PackedBitSet8[pageCount];
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public ref PackedBitSet8 GetPageByIndex(int pageIndex)
        {
